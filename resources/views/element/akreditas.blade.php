            ...
            <div class="card-body">
                ...
                <form action="/element/put-akreditasi/{{ $element->id }}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control" name="score" value="{{ ltrim($element->score_hitung, 0) }}" hidden>
                        @else
                        <input class="form-control" value="{{ $element->score_hitung }}" disabled>
                        <input class="form-control" name="score" value="{{ $element->score_hitung }}" hidden>
                        @endif
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control" name="min" {{ @(Auth::user()->role == 'Auditor') ? 'readonly' :''}} required>
                    </div>
                    <div class="form-group">
                        <button class="btn-primary btn-sm" {{ @(Auth::user()->role == 'Auditor') ? 'disabled hidden' :''}} type="submit">Simpan</button>
                    </div>
                </form>
            </div>
        </div>
